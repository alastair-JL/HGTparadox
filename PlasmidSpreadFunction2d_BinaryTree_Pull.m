function [Trajectory,timeVector] = PlasmidSpreadFunction2d_BinaryTree_Pull(Q,k,q,mutationRate,Jumps,EnviroShape,SharingNess,baseFit,baseDeath,FitDeltaLin,FitDeltaDeath,FitDeltaCombo,simID,ChangeTime,InitialRatios)
...
elseif(ActionPick==2) %%Mutation
   ...
   toRefresh=NextIndex; 
    ...
    Reciever= mod(NextIndex+Jumps(randi(length(Jumps)))-1,N^2)+1;
    if(ChromosoneType(Reciever)>0)
        PlasmidType(NextIndex,ActionPick-2)=PlasmidType(Reciever,ActionPick-2);
        toRefresh=NextIndex;
