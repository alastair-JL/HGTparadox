function [Trajectory,timeVector] = PlasmidSpreadFunction2d_BinaryTreeGradient(Q,k,q,mutationRate,Jumps,EnviroShape,SharingNess,baseFit,baseDeath,FitDeltaLin,FitDeltaDeath,FitDeltaCombo,simID,ChangeTime,InitialRatios)
...
elseif(EnviroShape==2)
    Environment= min(max((mod(1:N^2,N)*2-N/2)/N,0),1)'; %Random Environment. Yay!
...
FullIndex=([PlasmidType(toRefresh,:)-1,zeros(length(toRefresh),1)]*PlasmidMultiplier+1).*(ChromosoneType(toRefresh)>0)+1*(ChromosoneType(toRefresh)==0);
Rates=[fitnessBox(FullIndex).*Environment(toRefresh)+(1-Environment(toRefresh)).*fitnessBox(FullIndex+PlasmidMultiplier(end))...
    ,DeathRate(FullIndex).*Environment(toRefresh)+(1-Environment(toRefresh)).*DeathRate(FullIndex+PlasmidMultiplier(end)),mutationRate*ones(length(FullIndex),1),SharingNess(ChromosoneType(toRefresh)+1)'*ones(1,q)]';
...
if(~isempty(toRefresh))
   ...
   if(ChromosoneType(toRefresh)==0)
       ...
       FullIndex=([PlasmidType(toRefresh,:)-1,zeros(length(toRefresh),1)]*PlasmidMultiplier+1).*(ChromosoneType(toRefresh)>0)+1*(ChromosoneType(toRefresh)==0);
        Rates(:,toRefresh)=[fitnessBox(FullIndex).*Environment(toRefresh)+(1-Environment(toRefresh)).*fitnessBox(FullIndex+PlasmidMultiplier(end))...
            ,DeathRate(FullIndex).*Environment(toRefresh)+(1-Environment(toRefresh)).*DeathRate(FullIndex+PlasmidMultiplier(end)),mutationRate*ones(length(FullIndex),1),SharingNess(ChromosoneType(toRefresh)+1)'*ones(1,q)]';
   end
